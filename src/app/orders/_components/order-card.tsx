...
function getDisplayId(order: Order): string {
  if (order.tracking_id) {
    return order.tracking_id.slice(0, 8).toUpperCase();
  }
      ...
      <CardContent className="p-6">
                      ...
                      {hasItemDiscount ? (
                        <>
                          <p className="text-xs text-gray-400 line-through">
                            {formatCurrency(item.original_price!, order.currency)}
                          </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button asChild variant="outline" size="sm" className="py-4 transition transform font-bold text-white bg-blue-600 hover:bg-blue-400">
...
const OrderDetailsModal: React.FC<{ order: Order }> = ({ order }) => {
  ...
  // Calculate original vs discounted items total
  ...
  const hasProductDiscount = originalItemsTotal > discountedItemsTotal;
                      ...
                      <div className="text-right">
                        ...
                        {item.discount_applied > 0 && (
                          <p className="text-sm text-green-600">
                            -{formatCurrency(item.discount_applied, order.currency)}{" "}
                            discount
                          </p>
                        )}
                      </div>
